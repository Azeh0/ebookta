// ONNX Runtime is loaded via CDN script tag, available as window.ort
const ort = window.ort;

if (!ort) {
    console.error('ONNX Runtime not available! Ensure the script tag is loaded before this module.');
} else {
    // Configure WASM paths to use CDN - this ensures the WASM files are loaded correctly
    ort.env.wasm.wasmPaths = 'https://cdn.jsdelivr.net/npm/onnxruntime-web/dist/';
    
    // Disable verbose logging for better performance (set to true for debugging)
    ort.env.debug = false;
    ort.env.logLevel = 'warning';
    
    console.log('ONNX Runtime Web configured, version:', ort.env.versions?.web || 'unknown');
}
 ...
 */
export function configureEnv(settings) {
    if (!ort) {
        console.error('ONNX Runtime not loaded');
        return;
    }
    if (settings.numThreads) {
        ort.env.wasm.numThreads = settings.numThreads;
    }
    console.log('ONNX Environment Configured:', settings);
}
...
export class TextToSpeech {
    ...
    async _infer(textList, style, totalStep, speed = 1.05, progressCallback = null) {
        ...
        for (let step = 0; step < totalStep; step++) {
            ...
            // Yield to browser every step to keep UI responsive
...
/**
 * Load ONNX model with error handling and progress tracking
 */
export async function loadOnnx(onnxPath, options, modelName = 'Unknown') {
    const startTime = performance.now();
    
    try {
        console.log(`\n[${modelName}] Starting to load from: ${onnxPath}`);
        console.log(`[${modelName}] Options:`, JSON.stringify(options));
        
        // First, verify the file exists by fetching headers
        console.log(`[${modelName}] Checking if model file is accessible...`);
        const headResponse = await fetch(onnxPath, { method: 'HEAD' });
        if (!headResponse.ok) {
            throw new Error(`Model file not found or inaccessible: HTTP ${headResponse.status}`);
        }
        const fileSize = headResponse.headers.get('content-length');
        console.log(`[${modelName}] Model file found, size: ${fileSize ? (parseInt(fileSize) / 1024 / 1024).toFixed(2) + ' MB' : 'unknown'}`);
        
        // Now create the inference session
        console.log(`[${modelName}] Creating inference session...`);
        const session = await ort.InferenceSession.create(onnxPath, options);
        
        const loadTime = ((performance.now() - startTime) / 1000).toFixed(2);
        console.log(`[${modelName}] ✓ Successfully loaded in ${loadTime}s`);
        console.log(`[${modelName}]   - Input names: ${session.inputNames.join(', ')}`);
        console.log(`[${modelName}]   - Output names: ${session.outputNames.join(', ')}`);
        
        return session;
    } catch (err) {
        const loadTime = ((performance.now() - startTime) / 1000).toFixed(2);
        console.error(`[${modelName}] ✗ Failed after ${loadTime}s:`, err);
        console.error(`[${modelName}] Error details:`, {
            name: err.name,
            message: err.message,
            stack: err.stack
        });
        throw new Error(`Failed to load ${modelName} from ${onnxPath}: ${err.message}`);
    }
}

/**
 * Load all TTS components with detailed progress tracking
 */
export async function loadTextToSpeech(onnxDir, sessionOptions = {}, progressCallback = null) {
    const totalStartTime = performance.now();
    console.log('\n========================================');
    console.log('   TTS MODEL LOADING STARTED');
    console.log('========================================');
    ...
    // Re-fetch for use (loadCfgs doesn't return, we need to fetch again)
    const cfgs = await loadCfgs(onnxDir);
    ...
    console.log('\n[Step 2/6] Loading ONNX models...');
    console.log('  Models to load:', modelPaths.map(m => m.name).join(', '));
    
    const sessions = [];
    for (let i = 0; i < modelPaths.length; i++) {
        const stepNum = i + 2; // Steps 2-5 for the 4 models
        console.log(`\n[Step ${stepNum}/6] Loading ${modelPaths[i].name}...`);
        ...
        try {
            const session = await loadOnnx(modelPaths[i].path, sessionOptions, modelPaths[i].name);
            sessions.push(session);
            console.log(`  ✓ ${modelPaths[i].name} loaded successfully`);
        } catch (err) {
            console.error(`  ✗ Failed to load ${modelPaths[i].name}:`, err);
            throw err;
        }
    }
    ...
    console.log('\n[Step 6/6] Loading text processor...');
    try {
        ...
        console.log('\n========================================');
        console.log(`   TTS MODEL LOADING COMPLETE (${totalTime}s)`);
        console.log('========================================');
        console.log('Sample Rate:', textToSpeech.sampleRate);
        console.log('Ready for text-to-speech generation!');
        console.log('========================================\n');
        
        return { textToSpeech, cfgs };
    } catch (err) {
        console.error('  ✗ Failed to load text processor:', err);
        throw err;
    }
}
